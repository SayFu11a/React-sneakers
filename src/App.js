  ...
  const onAddToCart = async (obj) => {
    try {
      ...
      if (filtredItem) {
        ...
        await axios.delete(`https://63a826327989ad3286fb1b90.mockapi.io/cart/${filtredItem.id}`)
      } else {
        setCartItem((prev) => [...prev, obj]);
        const { data } = await axios.post('https://63a826327989ad3286fb1b90.mockapi.io/cart', obj);
        setCartItem((prev) =>
          prev.map((item) => {
  ...
  const onAddToFavorite = async (obj) => {
    try {
      if (favorites.find((favObj) => favObj.id == obj.id)) {
        ...
        const { data } = await axios.post('https://63ca4346d0ab64be2b4f3f3c.mockapi.io/Favorite', obj)
